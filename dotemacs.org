...
#+BEGIN_SRC emacs-lisp
  ...
  (defface thblt/spaceline-window-information-face
    '((t :foreground "orange" :background "black" :weight bold))
    "")

  (defface thblt/spaceline-window-information-face-active
    '((t :inherit thblt/spaceline-window-information-face :inverse-video t))
    "")

  (defface thblt/spaceline-persp-face
    '((t :background "DarkViolet" :foreground "white" :weight bold))
    "")

  (defface thblt/spaceline-persp-face-buffer-not-in-persp
    '((t :background "red" :foreground "white" :weight bold))
    "")

  (defface thblt/spaceline-buffer-id-face
    '((t :background "DeepSkyBlue" :foreground "#444444"))
    "")

  (spaceline-define-segment thblt/window-information
    ""
    (progn
      (aw-update)
      (window-parameter (selected-window) 'ace-window-path))

    :face thblt/spaceline-window-information-face-active)

  (spaceline-define-segment thblt/persp-name
    "The current perspective name."
    (when (or (not (string= persp-nil-name (safe-persp-name (get-frame-persp))))
              spaceline-display-default-perspective)

      (safe-persp-name (get-frame-persp)))
    :face      thblt/spaceline-persp-face)
  ;; @FIXME
  ;; (if t
  ;;             thblt/spaceline-persp-face
  ;;           thblt/spaceline-persp-face-buffer-not-in-persp))

  (spaceline-define-segment thblt/buffer-id
    ""
    (buffer-name)
    :face thblt/spaceline-buffer-id-face)

  (spaceline-compile 'thblt
    '(thblt/window-information
      thblt/persp-name
      thblt/buffer-id
      (flycheck-error :tight-left t)
      (flycheck-warning :tight-left t)
      (flycheck-info :tight-left t)
      ;; :face thblt/spaceline-buffer-id-face)
      )
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq erc-server "irc.freenode.net"
        ...
        erc-header-line-format (concat
                                (propertize "%t" 'face 'bold)
                                " "
                                (propertize "%o" 'face 'italic)))

...
#+BEGIN_SRC emacs-lisp
  (defun thblt/borg-git-electric-commit-message ()
    ...
    (when (equal
      ...
      (cl-flet ((plural (verb count &optional nosubject)
                        (if (zerop count)
                            ""
                          (format "%s %s%s"
                                  verb
        ...
        (let* ((status
                (cl-remove-if-not (lambda (p) (file-directory-p (cadr p)))
             ...
             (when (> count 1)
               (s-capitalize
                (concat
                 ...
                 (when (and assimilated modified) ", ")
                 (plural "upgrade" modified-c (not (zerop assimilated-c)))
                 "\n\n"

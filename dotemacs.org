...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'color-identifers-mode
    (add-to-list
...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun thblt/drones-status ()
    (interactive)
    (with-output-to-temp-buffer "*Drones*"

     (let* ((installed (borg-drones))
            (extra (cl-remove-if (lambda (x) (member x required-drones)) installed))
            (missing (cl-remove-if (lambda (x) (member x installed)) required-drones)))
       (print "* Extra drones:\n")
       (print (mapconcat 'identity extra ", "))

       (print "\n\n* Missing drones: ")
       (print (mapconcat 'identity missing ", ")) (fill-paragraph))))
  #+END_SRC

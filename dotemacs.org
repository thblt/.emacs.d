...
This is my Emacs configuration.  It is pretty much like any other, with maybe a few interesting properties:
 ...
 + It uses a [[https://en.wikipedia.org/wiki/Literate_programming][literate programming]] style with Org-Mode.  Although the documentation is admittedly scarce, it makes the configuration easy to organize and understand.

 + It uses the excellent [[https://github.com/emacscollective/borg][Borg]] package manager instead of anything based on =package.el=, including =use-package=.  Borg has a lot of interesting properties compared to the alternatives: since packages (“drones”) are Git submodules, they can be modified and contributed back.  Submodules also guarantee excellent configureation reproducibility, since you manually configure which version of which package you want installed, and version here means a /commit/ --- you don't get finer-grained than that.

 + It has a whole section dedicated to [[#writing-prose][writing prose]].

 + It uses [[https://github.com/abo-abo/swiper][Ivy]] /and/ (a bit of) [[https://github.com/emacs-helm/helm][Helm]].  The bulk of completion is performed through Ivy, but in some rare cases where Helm is still required, or is a better fit for the job, Helm is used instead.
...
#+BEGIN_SRC emacs-lisp
  (defun thblt/powerline-set-faces (&rest args)
    (let* ((default-bg (face-attribute 'default :background))
     ...
     (bg-down1 (kurecolor-adjust-brightness default-bg (* 1 (- brightness-step))))
     (bg-down2 (kurecolor-adjust-brightness default-bg (* 2 (- brightness-step))))
     (bg-down4 (kurecolor-adjust-brightness default-bg (* 6 (- brightness-step))))
     (bg-up1 (kurecolor-adjust-brightness default-bg (* 1 brightness-step)))
     (bg-up2 (kurecolor-adjust-brightness default-bg (* 2 brightness-step)))
     (bg-up3 (kurecolor-adjust-brightness default-bg (* 6 brightness-step))))
...
The modelines of the windows at the bottom left and the bottom right of the frame display special information:
...
There's no /fast/ way to determine a window's position when rendering the mode-line, so we need to store them statically in the window parameters, and update on ~window-configuration-change-hook~:
  ...
  (setq-default
   ...
   '((:eval
      (when (and (bound-and-true-p eyebrowse-mode) ;; Eyebrowse is bound and on
                 (< 1 (length (eyebrowse--get 'window-configs))) ;; We have configs
        ...
        (let* ((num (eyebrowse--get 'current-slot))
          ...
          (propertize (thblt/modeline-wrap str) 'face '(:weight bold :background "#FFCC33" :foreground "black" :box t)))))
     ...
     (:eval (when vc-mode (moody-tab (substring vc-mode 1) nil 'up)))
     " "
     (:eval
      (when (and t ;; @FIXME
                 (window-parameter (selected-window) 'thblt/window-at-bottom-right))
        (moody-wrap server-name 1 'up 'ribbon
                    'thblt/mode-line-server-name-face
                    'thblt/mode-line-server-name-face)))))
#+END_SRC
...
Since the author of Smartparens released [[https://github.com/Fuco1/.emacs.d/blob/master/files/smartparens.el][his own config]], here it is, copy-pasted and slightly modified to suit my needs:
...
#+BEGIN_SRC emacs-lisp
  ...
  This function is meant to be run as a hook in `git-commit-mode'."
    ...
    (let ((default-directory (vc-git-root default-directory)))
      (--when-let (cl-some 'funcall thblt/git-auto-commit-message-functions)
        ;; (insert it))))
        (insert it))))
...
#+BEGIN_SRC emacs-lisp
  (defun thblt/borg-git-auto-commit-message ()
  ...
  (with-eval-after-load 'magit
    (add-to-list 'thblt/git-auto-commit-message-functions 'thblt/borg-git-auto-commit-message))
#+END_SRC

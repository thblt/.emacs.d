...
with maybe a few interesting properties:
 ...
 + It uses the excellent [[https://github.com/emacscollective/borg][Borg]] package manager instead of anything
   ...
   back.  Submodules also guarantee excellent configureation
      ...
      (face-spec-set 'thblt/mode-line-server-name-face
                     `((t
                        ...
                        :background ,(kurecolor-hsv-to-hex
                                     .85
                                     .95
                                     (kurecolor-hex-get-brightness bg-up3))
                        :box ,default-fg)))

      ;; This is for Moody, which needs underline and overline.
      (face-spec-set 'thblt/mode-line-invisible
                     `((t
...
The mode-lines of the windows at the bottom left and the bottom right of the frame display special information:
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq-default
   ...
   '((:eval
     ...
     (:eval
      (when vc-mode
        (moody-tab (substring vc-mode 1) nil 'up)))

     (:eval (when vc-mode "   ")) ;; Not the cleanest way to add a conditional separator...
...
A lot of packages add overlays which are useful when editing, noisy when reading.  We advise scpaste so a few minor modes get disabled before it runs, and restored afterwards.

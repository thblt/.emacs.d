...
Garbage collection happens waaaay too early, and causes noticeable lags.  We set the threshold to the higher possible value, but trigger the GC automatically each time a frame loses focus.
...
Correct words using Helm instead of default method:
...
#+begin_src emacs-lisp
  ...
  (setq
   ;; Use ivy
   mu4e-completing-read-function 'helm-completing-read
